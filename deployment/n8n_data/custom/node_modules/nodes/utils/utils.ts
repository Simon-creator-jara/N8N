...
import type { RedisClient, RedisCredential } from './types';

export function getN8NRedisCredentials(): RedisCredential {
    return {
        host: process.env.QUEUE_BULL_REDIS_HOST || '',
        port: parseInt(process.env.QUEUE_BULL_REDIS_PORT || '6379'),
        password: process.env.QUEUE_BULL_REDIS_PASSWORD,
        ssl: (process.env.QUEUE_BULL_REDIS_TLS || 'false') === 'true',
        user: process.env.QUEUE_BULL_REDIS_USERNAME,
        database: 0,
        disableTlsVerification: (process.env.QUEUE_BULL_REDIS_TLS || 'false') === 'false'
    }
}
...
export async function setValue(
  ...
  if (type === undefined || type === 'automatic') {
    ...
    if (typeof value === 'string') {
      ...
      throw new ApplicationError('Could not identify the type to set. Please set it manually!', {
        level: 'error',
      });
    }
  }
    ...
    if (valueIsJSON) {
      ...
      for (const key of Object.keys(values as object)) {
        await client.hSet(
          keyName,
          key,
          (values as IDataObject)[key]!.toString(),
        );
      }
    } else {
...
export const flattenKeys = (
  ...
  return !isObject(obj)
    ...
    : reduce(
        obj,
        (cum, next, key) =>
          merge(cum, flattenKeys(next as IDataObject, [...prefix, key])),
        {},
      );
};
...
export function addExecutionHints(
  ...
  if (
    ...
    (context.getNodeParameter(
      'options.queryBatching',
      0,
      'single',
    ) as string) === 'single' &&
    (context.getNodeParameter('query', 0, '') as string)
      .toLowerCase()
      .startsWith('insert')
  ) {
    context.addExecutionHints({
