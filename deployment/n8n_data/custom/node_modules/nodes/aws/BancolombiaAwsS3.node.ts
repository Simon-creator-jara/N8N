...
export class BancolombiaAwsS3 implements INodeType {
  description: INodeTypeDescription = {
    displayName: 'Bancolombia Aws S3',
    name: 'bancolombiaAwsS3',
    icon: {
    ...
    description: 'Sends objects to and manages objects in AWS S3',
    defaults: {
      name: 'bancolombiaAwsS3',
    },
    usableAsTool: true,
      ...
      try {
        if (resource === 'folder') {
          ...
          if (operation === 'create') {
            ...
            const servicePath = bucketName.includes('.')
              ? 's3'
              : `${bucketName}.s3`;
          ...
          if (operation === 'getAll') {
            ...
            if (Array.isArray(responseData)) {
              responseData = responseData.filter(
                (e: IDataObject) =>
                  (e.Key as string).endsWith('/') &&
                  e.Size === '0' &&
                  e.Key !== options.folderKey,
              );
              if (qs.limit) {
        ...
        if (resource === 'file') {
          ...
          if (operation === 'copy') {
            ...
            const executionData = this.helpers.constructExecutionMetaData(
              this.helpers.returnJsonArray(
                responseData.CopyObjectResult as IDataObject[],
              ),
              { itemData: { item: i } },
            );
            returnData.push(...executionData);
          ...
          if (operation === 'getAll') {
            ...
            if (Array.isArray(responseData)) {
              responseData = responseData.filter(
                (e: IDataObject) =>
                  !(e.Key as string).endsWith('/') && e.Size !== '0',
              );
              if (qs.limit) {
                responseData = responseData.splice(0, qs.limit as number);
              }
          ...
          if (operation === 'upload') {
            ...
            const tagsValues = (
              this.getNodeParameter('tagsUi', i) as IDataObject
            ...
            if (isBinaryData) {
              ...
              const binaryPropertyData = this.helpers.assertBinaryData(
                i,
                binaryPropertyName,
              );
              ...
              if (binaryPropertyData.id) {
                ...
                const uploadId =
                  createMultiPartUpload.InitiateMultipartUploadResult.UploadId;
                let part = 1;
                for await (const chunk of uploadData) {
                  const chunkBuffer = await this.helpers.binaryToBuffer(
                    chunk as Readable,
                  );
